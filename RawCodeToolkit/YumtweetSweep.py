...
import time
import nltk, MySQLdb
consumer_key = 'JAYp4AEX9FsrkaP95TVNsw'
consumer_secret = '3yvQHV2CRhPkdjaspYYPzxN8ZCZLab5hu4vAOWOcZXI'
access_token = '52586641-ySj9BuZAClmcZgHWnayLmzVHaVczQOd6hSYOCiQCM'
access_token_secret = 'KbKC3rFE1taYPzbp1rBlnzDMubLa42hAXIs6WCrOhc8VQ'
...
timespan = timespan - datetime.timedelta(days=50)
...
for tweet in tweepy.Cursor(api.search, q="Atco", rpp=100, until="2014-02-08",include_entities=True, lang="en").items():
    if(limit <= 5):
        status = api.rate_limit_status()
        limit = status['resources']['search']['/search/tweets']['remaining']
        if(limit <= 5):
    ...
    print("Create: {0} Timespan: {1}".format(str(tweet.created_at),str(timespan)))

#    if tweet.created_at < timespan:
#    if ((tweet.created_at - timespan) <= datetime.timedelta(days = 0)):
#        break
