...
# TweetSweep is a script used for minning tweets containing the specified query term
# (company name or alias). It stores all the found tweets in the database for 
# later sentiment parsing.
import tweepy
import datetime
import calendar
import time
...
# Set twitter required keys & tokens
...
# Initially, we tried to limit our search range but it turns out 
# that twitter imposes about a 6-8 day max range on fetchable tweets.
...
# Twitter imposes a max query rate,
# if we get close to exceeding it, sleep.
